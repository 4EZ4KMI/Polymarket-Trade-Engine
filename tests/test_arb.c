...
PT_T(arb_edge_calculation)
{
    ...
    PT_ASSERT_NEAR(opp.raw_edge, 0.050, 0.0001);
    ...
    PT_ASSERT(opp.expected_net_profit > 0.0);
}
...
PT_T(arb_partial_fill_protection)
{
    ...
    /* Must require hedge or abort */
    PT_ASSERT(act.action == PT_ARB_ACT_MARKET_HEDGE || act.action == PT_ARB_ACT_ABORT);
}
